...
class Encoder(nn.Module):
    ...
    def __init__(self, *args, **kwargs):
        ...
        """
        (1, 36, 128) => (5, 36, 128) => (10, 18, 64) => (10, 9, 32) => (8, 1, 32)
        (k-s)/2 = p
        """
...
class Encoder2(nn.Module):
    ...
    def __init__(self, *args, **kwargs):
        ...
        C_structure = [5, 10, 10, self.latent_dim]
        k_structure = [(3,9), (4,9), (4,9), (9,9)]
        s_structure = [(1,1), (2,1), (2,1), (9,1)]
...
class Decoder2(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        """
        (8, 1, 128) => (10, 9, 128) => (10, 18, 128) => (5, 36, 128) => (1, 36, 128)
        (k-s)/2 = p
        """
        C_structure = [10, 10, 5, 1]
        k_structure = [(9,9), (4,9), (4,9), (3,9)]
        s_structure = [(9,1), (2,1), (2,1), (1,1)]
...
class LangClassifier(nn.Module):
    def __init__(self, *args, **kwargs):
        """
        new : (8, 1, 32) => (1, 8, 32) => (8, 4, 32) 
        => (16, 2, 32) => (32, 2, 32) => (16, 2, 32)  => (196, 1, 32)
        """
        ...
        self.conv3 = Conv2d_GLU(inC=16, outC=32, k=(2,3), s=(2,1), p=(1,1))
        self.conv4 = Conv2d_GLU(inC=32, outC=16, k=(2,3), s=(2,1), p=(1,1))

        self.conv_out = nn.Conv2d(16, self.label_num, (2,1), (1,1), padding=(0,0))
        self.label_out = nn.LogSoftmax(dim=1)
...
class LangClassifier2(nn.Module) : 
    def __init__(self, *args, **kwargs):
        super(LangClassifier2, self).__init__()
        """
        (8, 1) => (8) => (16) => (32) => (64) => (128) => (196) => (196)
        ...
        self.fc5 = torch.nn.Linear(128, 196)
        self.fc6 = torch.nn.Linear(196, self.label_num)

    def step(self, x_) :
        x_ = x_.view(x_.shape[0], x_.shape[1])
        h1 = self.fc1(x_)
        h1_ = F.relu(h1)
        h2 = self.fc2(h1_)
        h2_ = F.relu(h2)
    ...
    def forward(self, x) : 
        ...
        for i in range(x.shape[-1]) : 
            step_o = self.step(x[:, :, :, i])
            outputs.append(step_o)

        return outputs

class LangClassifier128(nn.Module):
    def __init__(self, *args, **kwargs):
        """
        new : (8, 1, 1) => (1, 1, 8) 
        => (8, 1, 8) 
        ...
        super(LangClassifier128, self).__init__()
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.latent_dim = kwargs.get("latent_dim", 0)
        self.label_num = kwargs.get("label_num", 4)
    
        self.conv1 = Conv2d_GLU(inC=1, outC=8, k=(1,1), s=(1,1), p=(0,0))
        self.conv2 = Conv2d_GLU(inC=8, outC=16, k=(1,2), s=(1,2), p=(0,0))
        self.conv3 = Conv2d_GLU(inC=16, outC=32, k=(1,1), s=(1,1), p=(0,0))
        #self.conv4 = Conv2d_GLU(inC=64, outC=128, k=(2,1), s=(2,1), p=(1,1))

        self.conv_out = nn.Conv2d(32, self.label_num, (1,1), (1,1), padding=(0,0))
        self.label_out = nn.LogSoftmax(dim=1)
...
class LangClassifierFinal(nn.Module):
    def __init__(self, *args, **kwargs):
        """
        new : (8, 1, 32) => (1, 8, 32) => (8, 4, 32) 
        => (16, 2, 32) => (32, 2, 32) => (47, 1, 32)
        """

        super(LangClassifierFinal, self).__init__()
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.latent_dim = kwargs.get("latent_dim", 0)
        self.label_num = kwargs.get("label_num", 4)
        ...
        self.conv_out = nn.Conv2d(32, self.label_num, (2,1), (1,1), padding=(0,0))
        self.label_out = nn.LogSoftmax(dim=1)
...
class LangClassifierFinal_MLP(nn.Module):
    def __init__(self, *args, **kwargs):
        """
        new : (8, 1, 32) => (256) => (512) 
        => (1024) => (1504) => (47, 1, 32)
        ...
        super(LangClassifierFinal_MLP, self).__init__()
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.latent_dim = kwargs.get("latent_dim", 0)
        self.label_num = kwargs.get("label_num", 4)
...
class VAE2(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.enc = Encoder2(style_dim=self.style_dim, latent_dim=self.latent_dim,vae_type=self.vae_type, weight_sharing=self.weight_sharing)
        self.dec = Decoder2(style_dim=self.style_dim, latent_dim=self.latent_dim,vae_type=self.vae_type, weight_sharing=self.weight_sharing)
